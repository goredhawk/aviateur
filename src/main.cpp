...
public:
    ...
    void start_playing(std::string url) {
        playing_ = true;
        player_->play(url, url.starts_with("udp") ? true : force_software_decoding);
...

            {
...

                {
                    ...
                    auto callback = [this](uint32_t) {
                        channel = std::stoi(channel_button_->get_selected_item_text());
                    };
...
int main() {
    ...
    auto onWifiStop = [render_rect_raw, control_panel_raw] {
        ...
        control_panel_raw->update_adapter_start_button_looking(true);
    };
